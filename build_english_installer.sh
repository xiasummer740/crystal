...
echo ">>> 1. 正在重构 100% 纯英文 ASCII 防爆版部署脚本..."
...
chmod +x install.sh
# 强制转换为 Unix 换行符，防止任何回车符污染
sed -i 's/\r$//' install.sh

echo ">>> 2. 正在强行提交至 GitHub (crystal 分支)..."
git add install.sh
git commit -m "🚀 [Ops] Rewrite install.sh to 100% Pure ASCII English to prevent locale syntax errors on bare metal VPS" || true
git push origin crystal -f

echo "========================================================="
echo " ✅ 纯英文防爆装甲版 install.sh 已强推至 GitHub！"
echo "========================================================="
