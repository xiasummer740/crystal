...
echo ">>> 1. 正在重构后端：解除大模型产品数量封印，升级高感度图片抓取..."
...
const searchLead = async (req, res) => {
    try {
        ...
        const [
            ...
            fetchMatrixQuery(`${siteQueryStr} "产品中心" OR "型号" OR "系列"` , 15), // 抓取15条，疯狂收集产品型号
            fetchMatrixQuery(`${siteQueryStr} "解决方案" OR "技术参数"`),
            fetchMatrixQuery(`${exact} (site:1688.com OR site:hc360.com) "主营产品" OR "参数"`),
            fetchMatrixQuery(`${exact} (site:elecfans.com OR site:hqew.com) "产品"`),
            fetchMatrixQuery(`${exact} 招聘 硬件工程师`),
            fetchMatrixQuery(`${exact} 招标 采购项目清单`)
        ]);

        // 【无限火力 Prompt】：强制解除产品数量限制
...
【无限量产品提取铁律 - 绝不允许遗漏】：
1. **必须把你在情报中看到的所有【具体硬件产品名称/型号】全部提取出来！有多少写多少，上不封顶！不要只写几个凑数！** (至少要写 6-10 款，如果数据里有的话)
2. **产品BOM绝对禁止空白**：必须详细列出该设备的【全景芯片架构】。如果没写，必须依据行业常识强行推演（标明推演）。
3. **小白解说**：在 \`function\` 字段中，必须包含：“为什么需要它：xxx。如果没有它：会xxx。”
...
输出严格JSON:
{
  ...
  "products": [
    {
      "name": "具体设备名称或型号(有多少列多少！不封顶！)",
      "chipPlatform": "芯片架构组合(必须使用文字连贯表述，如：主控采用xxx，通信采用xxx，严禁留白)",
      "crystals": [
        {"freq": "频率", "package": "封装", "loadCap": "电容", "tolerance": "频偏", "function": "小白解说：为什么需要它？如果没有它会怎样？"}
      ]
    }
  ],
  "strategy": "基于原厂晶振销售的极度硬核切入策略"
        ...
        const response = await fetch(targetUrl, {
            ...
            body: JSON.stringify({
                ...
                temperature: 0.3, // 稍微升温，给 AI 更多拼接发散空间提取产品
        ...
        if (resultData.products && Array.isArray(resultData.products)) {
            const enrichedProducts = await Promise.all(resultData.products.map(async (p) => {
                ...
                // 放弃 site: 语法，改用强关联词搜索，命中率极大幅度提升
                const query = `"${resultData.company}" "${p.name.replace(/\(.*?\)/g, '').trim()}" 产品 官方实物图片`;
...
echo ">>> 2. 正在重写前端 UI：注入【半岛小芯与 PDF 查询直通车】..."

cat << 'EOF' > "$FRONTEND_DIR/src/components/ResultDisplay.jsx"
import React, { useState, useEffect } from 'react';
import axios from 'axios';
import { useStore } from '../store';
import { Building2, Cpu, Zap, Lightbulb, Link as LinkIcon, Info, Box, Layers, Copy, Check, MapPin, Star, ShoppingCart, Globe2, FileText } from 'lucide-react';
